...
// import { createNoise2D } from "https://cdnjs.cloudflare.com/ajax/libs/simplex-noise/4.0.0/simplex-noise.ts"; // const { createNoise2D } = require('simplex-noise');
...
function start() {
	...
	const value2dx = noise2D(5, 3);
	console.log(value2dx);
}
...
function initPhysicsWorld() {
	 ...
	 * 
	 * This function sets up the necessary components for the physics simulation.
	 * It creates a collision configuration, a collision dispatcher, an overlapping pair cache,
	 * a constraint solver, and the physics world itself. It also sets the gravity for the world.
	*/
	// Create a new instance of Ammo.btDefaultCollisionConfiguration() to define the collision configuration
	let collisionConfiguration = new Ammo.btDefaultCollisionConfiguration();

	// Create a new instance of Ammo.btCollisionDispatcher() with the collision configuration to handle collision detection and resolution
	let dispatcher = new Ammo.btCollisionDispatcher(collisionConfiguration);

	// Create a new instance of Ammo.btDbvtBroadphase() as the overlapping pair cache for the broadphase collision detection
	let overlappingPairCache = new Ammo.btDbvtBroadphase();

	// Create a new instance of Ammo.btSequentialImpulseConstraintSolver() as the solver for constraint resolution
	let solver = new Ammo.btSequentialImpulseConstraintSolver();

	// Create a new instance of Ammo.btDiscreteDynamicsWorld() to represent the physics world
	// It takes the dispatcher, overlapping pair cache, solver, and collision configuration as parameters
	...
	// Set the gravity for the physics world using Ammo.btVector3(x, y, z) where (0, -10, 0) represents downward gravity
	physicsWorld.setGravity(new Ammo.btVector3(0, -10, 0));

	// Log a message to indicate that the physics world has been initialized
...
function initGraphicsWorld() {
	...
	(async function() {
		...
		let envmapTexture = await new RGBELoader()
			...
			.loadAsync("assets/lilienstein_4k.hdr");
		...
		for (let i = -15; i <= 15; i++) {
			for (let j = -15; j <= 15; j++) {
				...
				noise = (noise2D(i * 0.1, j * 0.1) + 1) * 0.5;
				...
				createCylinder(
					1,
					MAX_HEIGHT * noise * 1,
					new Vector3(position.x, MAX_HEIGHT * noise * 0.5, position.y),
					0,
					0xffffff,
					{ x: 0, y: 0, z: 0, w: 1 }
				);
...
function createPlayer(gltfModel) {
	...
	playerLoader.load(gltfModel, function(gltf) {
		...
		playerTransform.setOrigin(
			new Ammo.btVector3(playerPosition.x, playerPosition.y, playerPosition.z)
		);
		...
		structColShape.calculateLocalInertia(playerMass, playerLocalInertia);
...
function addPhysics(radius, height, position, mass, quaternion, factor) {
	...
	let colShape = new Ammo.btCylinderShape(
		new Ammo.btVector3(radius * factor, height * 0.5, radius * factor)
	);
...
function createCylinder(radius, height, position, mass, color, quaternion) {
	...
	newCylinder.name = `Physics-Mesh-${Math.floor(position.x) * -1}-${Math.floor(position.z) * -1
		}`;
